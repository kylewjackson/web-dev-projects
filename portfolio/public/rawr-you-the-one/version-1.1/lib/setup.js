...
const names = {
  ...
  //     'Fish'
  // ],
  owners: ['Sigfried', 'Steve', 'Dave', 'Jane', 'Veronica', 'Opal', 'Cesar', 'Tina', 'Thora', 'Josh', 'Arthur', 'Mowgli', //v1.1 name too long
  // 'Christopher',
  'Eliza', 'Buffy'],
  pets: ['Pup', 'Kitty', 'Parrot', 'Tiger', 'Bear', 'Chimp', 'Croc', 'Goat', 'Echidna', 'Hamster', 'Pig', 'Bunny', 'Snake', 'Toad'],
  cased: {
    lower: {},
    upper: {}
  },
...
function Game(rounds) {
  this.round = 1;
  this.rounds = rounds; //create shallow copies of owner/pet arrays in order to randomly assign pairs to the game

  const initOwners = [...names.owners];
  const initPets = [...names.pets];
  this.owners = [];
  this.pets = [];
  this.alphaPets; //splice out a random item from each initial array, and stop once new array is full for rounds
  ...
  this.matches = {}; //set up game matches
...
function setupNavbar(rounds) {
  ...
  clearChildElements(document.querySelector('header')); //remove start class from header

  classChange(document.querySelector('header'), [], ['start']); //remove config

  clearChildElements(document.querySelector('#config')); //add navbar
...
function setupConfig(rounds, parent) {
  if (parent === 'navbar') {
    ...
    document.querySelector(`.config-inputs[value="${rounds}"]`).checked = true; //set message variable
...
function setupLegendHeaders(game, adjust) {
  ...
  if (adjust && currentStats && currentStats.rows > 0) {
    ...
    let necessaryCells = 0; //set necessary cells if applicable
    ...
    while (document.querySelectorAll('.stat-selections.empty').length !== necessaryCells) {
      if (document.querySelectorAll('.stat-selections.empty').length < necessaryCells) {
        adjustEmptyCells(true, document.querySelectorAll('.stat-selections.empty').length + 1); //add list items based on current round
    ...
    for (let i = 0; i < game.rounds; i++) {
      ...
      if (document.querySelectorAll('.stat-selections')[i].getBoundingClientRect().left < cellPosition) {
        //push in a new set
        rowCount++;
        adjustLegendHeaders(true); //reset cell position
...
function adjustEmptyCells(add, i) {
  if (add) {
    document.querySelector('#stat-selections-container').insertAdjacentHTML('beforeend', blankCell); //set id of last element
