...
function startGame() {
    ...
    classChange(document.body, ['in-progress'], []);

    //reset valid woofs object
    ...
    currentStats = new Stats(currentGame);
    //create confirmed no match entry for each owner
    currentGame.owners.forEach(owner => {
        currentStats.confirmed.noMatch[owner] = [];
        //add owners into valid woof object
    ...
    //hide rules on start
    classChange(document.querySelector('#rules-container'), ['hidden'], []);

    //remove config and setup navbar
    setupNavbar(userRounds);
    ...
    setupStatsSection();
    setupOwnerLegend(currentGame);

    //hide stats on first round, and mark match as 'last'
    classChange(document.querySelector('#stats'), ['hidden'], []);
    classChange(document.querySelector('#nav-stats'), ['hidden'], []);
    classChange(document.querySelector('#nav-stats'), ['last'], []);

    //updateMessage(you have X rounds)
...
function checkWoofBooth(game, stats) {
    ...
    const woofedPet = document.querySelectorAll('.woof-inputs:checked')[1].value;

    //change heading display and hide inputs
    displayWoof(false, woofedOwner, woofedPet);

    //disable woof submit button
...
function woofNoMatch(game, stats, woofedOwner, woofedPet) {
    ...
    stats.round[game.round].woof.noMatch = [woofedOwner, woofedPet];

    //variables for no-match
    const matchedInput = document.querySelector(`#${woofedOwner}-${woofedPet}-input`);
    const matchedLabel = matchedInput.parentElement.querySelector(`.pet-labels.${woofedPet}`);
    const matchedSpan = matchedInput.parentElement.querySelector(`.pet-label-spans.${woofedPet}`);

    //disable/uncheck input for pairing
    matchedInput.disabled = true;
    if (matchedInput.checked) {
    ...
    //dim label
    dimLabels('noMatch', matchedLabel);

    //changeClass() mark span as no match
    classChange(matchedSpan, ['no'], []);

    //remove listings if applicable
    woofList(game, stats, woofedOwner, woofedPet, matchedSpan);

    //if stats chart, mark insances of pairing as no match
    if (stats.chart) {
...
function setupNewWoof(game, stats) {
    ...
    //reset submit button if set as match, and disable
    ...
    alphaWoofs.pets.sort();

    //insert each alphabetized valid selection into woof booth
    ...
    for (let i = 0; i < pets.length; i++) {
        if (!stats.confirmed.noMatch[owner].includes(pets[i])) {
            ...
            if (alphaPets.includes(pets[i])) {
                ...
                const previousOwner = alphaOwners[ownerIndex];
                //find another valid pair for the previously assigned owner
                for (let j = 0; j < pets.length; j++) {
                    ...
                    if (!stats.confirmed.noMatch[previousOwner].includes(pets[j]) && !alphaPets.includes(pets[j])) {
                        ...
                        alphaPets[ownerIndex] = pets[j];
                        //assign the current pet to the new owner
                        return [owner, pets[i]];
                    };
                };
            } else {
...
function potentialWoofInput(game, stats) {
    ...
    let woofChartPairing;

    //If there's already a potential woof marked, remove those classes
    ...
    if (stats.confirmed.match[ownerWoofInput.value] || stats.confirmed.noMatch[ownerWoofInput.value].length > 0) {
        if (stats.confirmed.match[ownerWoofInput.value] === petWoofInput.value ||
            stats.confirmed.noMatch[ownerWoofInput.value].includes(petWoofInput.value)) {
            ownerWoofInput.checked = false;
            petWoofInput.checked = false;
            //disable button
    ...
    //enable button
    document.querySelector('#submit-woof').disabled = false;

    // if stats charts, mark entries
    if (stats.chart) {
...
function changeSelection(game, stats, input) {
    ...
    //remove empty class from container if present
    ...
    dimLabels('pet', document.querySelectorAll(`input:not(:checked) + .pet-labels[id^="${ownerName}-"]`));

    //changeClasses(add) Marking pairing as active and un-dim if needed
    classChange(selectionSpan, ['active'], []);
    classChange(input.nextElementSibling, [], ['dimmer']);

    //mark uni/dupe selections and add passive for previous pet selection, and current pet selection
    ...
    passiveSelections(petName);

    //set selection in stats object
    stats.round[game.round].selections[ownerName] = petName;

    //check if submit should be enabled, if 4 inputs are checked and woof has been submitted
...
function markSelections(stats, pet) {
    ...
    const petArr = document.querySelectorAll(`.pet-label-spans.${pet}`);
    //array of all active instances of selected pet's active spans
    const activePetArr = document.querySelectorAll(`.pet-label-spans.${pet}.active-selection`);

    //loop through each selected pet span
    petArr.forEach(span => {
        ...
        if (span.classList.contains('active-selection') && activePetArr.length > 1) {
            ...
            classChange(span, ['dupe'], ['uni']);
            //mark header, and update header message
            ...
            classChange(span, ['uni'], ['dupe']);
            //mark header
            markHeader(span, ['uni'], ['dupe']);
            //collapse selections
            const ownerName = span.parentElement.previousElementSibling.name;
            const petName = span.parentElement.previousElementSibling.value;
            collapseSelections(ownerName, petName, false);
        ...
        } else { //if no pet spans are active
...
function collapseSelections(ownerName, petName, expand) {
    if (!expand) {
        ...
        classChange(document.querySelector(`#${ownerName}-heading`), ['collapsed'], []);
        //hide all elements except label for unique pet
        ...
        classChange(document.querySelector(`#${ownerName}-selections-container`), [], ['collapsed']);
        classChange(document.querySelector(`#${ownerName}-heading`), [], ['collapsed']);
        //unhide all elements except label for unique pet
        document.querySelectorAll(`#${ownerName}-radio-container > *:not(#${ownerName}-${petName}-label),
                #${ownerName}-${petName}-label > .selections-list-containers`).forEach(ele => {
            classChange(ele, [], ['hidden']);
        });
...
function passiveSelections(pet) {
    ...
    } else if (!document.querySelector(`.pet-label-spans.${pet}.active-selection`) &&
        document.querySelector(`.pet-label-spans.${pet}.passive-selection`)) { //if no other instance of pet is active, and there are passives
...
function checkWinner(game, stats) {
    ...
    const selectedPets = Object.keys(stats.round[game.round].selections);

    //check each round selection against current game matches, and break loop if no match
...
function endGame(game, stats, win) {
    ...
    //hide navigation
    classChange(document.querySelector('#nav-navigation-container'), ['hidden'], []);
    //expand restart if collapsed
...
function updateStats(game, stats) {
    ...
    //update round selections and check for matches
    roundSelections.forEach(selection => {
        currentRound.selections[selection.name] = selection.value;
        //check for matches
...
function blackout(game, stats, selections) {
    selections.forEach(selection => {
        ...
        const blackoutPet = selection.value;
        //update confirmed stats
        stats.confirmed.noMatch[blackoutOwner].unshift(blackoutPet);

        //mark pairs in match selections field and stat chart as no-match, disable/uncheck input
        //variables
        ...
        const blackoutStats = document.querySelectorAll(`.pet-stats[id$="-${blackoutOwner}-${blackoutPet}-selection"]`);
        ...
        classChange(blackoutLabel, ['dimmer'], []);
    ...
    stats.round[game.round].blackout = true;
    //update blackout count
    stats.blackouts++;
};
...
function resetRound(game, stats) {
    ...
    const petLabelSpans = document.querySelectorAll('.pet-label-spans');

    //changeClasses() remove all relevant classes (dupe, active, unique, passive, dimmer)
    if (petLabelSpans.length > 0) {
        petLabelSpans.forEach(span => {
            classChange(span, [], ['dupe', 'active', 'uni', 'passive']);
            //if no-match, don't remove dimmer
    ...
    if (stats.chart) {
        const statListItems = document.querySelectorAll('.pet-stats:not(.pawfect-match), .pet-stats:not(.no-match)')
        //changeClasses() remove all relevant classes (dupe, unique, dimmer)
...
function nextRound(game, stats) {
    ...
    setupNewWoof(game, stats);
    //change woof button text
    document.querySelector('#submit-woof').innerHTML = 'Throw me a bone, will ya?';
    //disable submit button and change text
    document.querySelector('#submit-selections').disabled = true;
    document.querySelector('#submit-selections').innerHTML = 'Make WoofBooth Selections';

    //increment round
    game.round++;

    //set attempt element on page to new round, and specify last round
...
function checkSubmit(game, stats, message) {
    ...
    if (document.querySelectorAll('.pet-label-spans.unique-selection').length +
        document.querySelectorAll('.pet-label-spans.pawfect-match').length === game.rounds) {
        ...
        if (document.querySelectorAll('.current-woof').length > 0) {
            ...
            //change text
            document.querySelector('#submit-selections').innerHTML = 'Submit';
            //change message
...
function setupStatChart(game, stats) {
    ...
    if (!stats.chart) {
        stats.chart = true;
        //remove last from nav-match
        classChange(document.querySelector('#nav-match'), [], ['last']);

        //unhide stats, and set nav-stats to last
        classChange(document.querySelector('#stats'), [], ['hidden']);
        classChange(document.querySelector('#nav-stats'), ['last'], ['hidden']);

        //add additional legend headers if necessary
    ...
    updateStatLists(game, stats);

    //add woofs and correct count
    ...
    document.querySelectorAll('.stat-woofs').forEach(element =>
...
function markStats(owner, pet, type) {
    ...
    const statPairings = document.querySelectorAll(`.pet-stats[id$="-${owner}-${pet}-selection"]`);
    //if multiple instances of pairing
...
function updateLists(input) {
    ...
    const listSelection = document.querySelector(`#${listOwner}-${listPet}-selection`);
...
function list(game, stats, owner, pet, kind) {
    ...
    const listStats = document.querySelectorAll(`.pet-stats[id$="-${owner}-${pet}-selection"]`);
    //If whitelist is checked on pairing in match field or stats field
    if (kind === 'white') {
...
function listCount(game, stats, owner, pet, kind, add) {
    if (add) {
        ...
        stats.list[kind].pet.push(pet);
        //add bullet into elements
        ...
        stats.list[kind].pet.splice(petIndex, 1);
        //remove one bullet from each instance (one bullet must already exist for it to be removed)
...
function navbarClicked(type) {
    ...
    document.querySelector(`#${type}-nav`).classList.toggle('hidden');
    //if either restart or nav is clicked, add clicked to message and header, otherwise remove it
