...
document.addEventListener('click', e => {
  ...
  if (e.target.matches('.list-inputs')) {
    ...
    const listPetName = e.target.parentElement.parentElement.previousElementSibling.value; //cycle through inputs
  ...
  if (e.target.matches('#reset-selections')) {
    resetRound(currentGame, currentStats); //update message
  ...
  if (e.target.matches('#message-container.message-ready')) {
    updateStats(currentGame, currentStats); //add stats chart, with woof result and number of correct matches

    setupStatChart(currentGame, currentStats); //if winner, execute winner function and end game
...
document.addEventListener('submit', e => {
  ...
  e.preventDefault(); //Start Game

  if (e.target.matches('#config-form')) {
    startGame(); //set timeout for first round
  ...
  if (e.target.matches('#selection-form')) {
    ...
    if (checkWinner(currentGame, currentStats)) {
      ...
      resetRound(currentGame, currentStats);
      nextRound(currentGame, currentStats); //v1.1 jump to stats section
